...
use crate::app::services::interaction_service::InteractionService;
use crate::app::services::thread_service::ThreadService;
...
impl Gateway {
    ...
    async fn handle_message(self: Arc<Self>, inbound: InboundMessage) -> Result<()> {
        ...
        let working_directory =
            ThreadService::new(self.default_working_directory.clone())
                .effective_working_directory(&current_thread);
        drop(_guard);
    ...
    async fn handle_interaction_command(
        ...
        match command {
            ...
            InteractionCommand::ReplyOther {
                ...
                let example = match &summary.kind {
                    PendingInteractionKind::UserInputRequest { questions }
                        if questions.len() == 1 =>
                    {
                        if pending.len() == 1 {
                            Some("/reply your answer".to_owned())
                        } else if let Some(ordinal) = ordinal {
                            Some(format!("/reply {ordinal} your answer"))
                        } else {
                            Some(format!("/reply {} your answer", summary.token))
                        }
                    }
                    PendingInteractionKind::UserInputRequest { .. } => {
                        let question_id = question_id.as_deref().unwrap_or("question_id");
                        if pending.len() == 1 {
                            Some(format!("/reply {question_id} your answer"))
                        } else if let Some(ordinal) = ordinal {
                            Some(format!("/reply {ordinal} {question_id} your answer"))
                        } else {
                            Some(format!("/reply {} {question_id} your answer", summary.token))
                        }
                    }
                    _ => None,
                };
                ...
                if let Some(example) = example {
                    self.adapter
                        .send(InteractionService::build_reply_other_notice(
                    ...
                    self.send_notice(
                        adapter_name,
                        conversation_id,
                        "This pending Codex request does not accept free-form input.".to_owned(),
                    )
                    .await?;
                }
            }
    ...
    async fn handle_management_command(
        ...
        let thread_service = ThreadService::new(self.default_working_directory.clone());
        match command {
            ManagementCommand::ListThreads => {
                let message = thread_service.format_threads_message(state);
                self.send_notice(adapter_name, conversation_id, message)
                    .await?;
            }
            ManagementCommand::NewThread(alias) => {
                let alias = alias.unwrap_or_else(|| thread_service.next_thread_alias(state));
                ...
                let inherited_directory = state
                    .current_thread_record()
                    .map(|thread| thread_service.effective_working_directory(thread))
            ...
            ManagementCommand::UseThread(alias) => {
                ...
                let working_directory = thread_service.effective_working_directory(thread);
    ...
    async fn load_state(&self, key: &str) -> ConversationState {
        ...
        ThreadService::new(self.default_working_directory.clone()).ensure_state_is_usable(&mut state);
        state
    }
